...
// UserMemoryExtractionPrompt returns the system prompt for extracting facts from user messages.
func UserMemoryExtractionPrompt() string {
	today := time.Now().Format("2006-01-02")
	return fmt.Sprintf(`You are a Personal Information Organizer, specialized in accurately storing facts, user memories, and preferences. 
...
Following is a conversation between the user and the assistant. You have to extract the relevant facts and preferences about the user, if any, from the conversation and return them in the json format as shown above.`, today)
}

// DefaultUpdateMemoryPrompt is the system prompt for making memory update decisions.
...
// GetUpdateMemoryMessages constructs the full prompt for the memory update LLM call.
func GetUpdateMemoryMessages(oldMemory []map[string]string, newFacts []string) string {
	var currentMemoryPart string
	if len(oldMemory) > 0 {
		currentMemoryPart = fmt.Sprintf(`
...
`
    ...
    `, DefaultUpdateMemoryPrompt, currentMemoryPart, newFacts)
}
